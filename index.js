...
async function fetchStockData() {
    ...
    try {
        const stockData = await Promise.all(tickersArr.map(async (ticker) => {
            const url = `https://polygon-api-worker.soymikey0717.workers.dev/?ticker=${ticker}&startDate=${dates.startDate}&endDate=${dates.endDate}`
...
async function fetchReport(data) {
    ...
    try {
        const url = 'https://openai-api-worker.soymikey0717.workers.dev'
